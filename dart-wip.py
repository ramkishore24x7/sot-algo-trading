...
def monitorHealth():
	...
	while Clock.time_in_range(startTime, endTime, datetime.datetime.now().time()):
		...
		# if len(BANKNIFTY_LTP) > 0 and len(NIFTY_LTP) > 0 and len(FINNIFTY_LTP) > 0:
		# 	print(f"{Clock.tictoc()} Nifty: @{NIFTY_LTP[-1]} || BankNifty: @{BANKNIFTY_LTP[-1]} || FinNifty: @{FINNIFTY_LTP[-1]} ", end="\r", flush=True)

		if len(BANKNIFTY_LTP) > 0 and len(NIFTY_LTP) > 0 and len(FINNIFTY_LTP) > 0:
			sys.stdout.write(f"{Clock.tictoc()} Nifty: @{NIFTY_LTP[-1]} || BankNifty: @{BANKNIFTY_LTP[-1]} || FinNifty: @{FINNIFTY_LTP[-1]} \r")
			sys.stdout.flush()
		time.sleep(2)
	os.system(Config.kill_banknifty_ws)
	os.system(Config.kill_nifty_ws)
