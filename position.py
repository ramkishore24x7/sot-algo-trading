...
class Position():
    def __init__(self,strike,entry_price,stoploss,target1,target2,target3,isBreakoutStrategy,enterFewPointsAbove,second_entry_price=None,onCrossingAbove=False,instrument=None,ce_pe=None) -> None:
        ...
        self.second_entry_price = second_entry_price if second_entry_price is not None else second_entry_price
        ...
        if ":" not in instrument:
            ...
            if match:
                ...
                if self.lot_size is None:
                    raise(f"Couldn't Resolve Lot Size for {match}")
                if self.freeze_quantity is None:
                    raise(f"Couldn't Resolve Freeze Quantity for {match}")
        else:
            self.lot_size = None
