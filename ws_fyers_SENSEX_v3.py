...
fyers = fyersModel.FyersModel(token=access_token,is_async=False,client_id=app_id,log_path=Config.fyers_log_path)
instrument_name = "SENSEX"
strikeList=['BSE:SENSEX-INDEX']
exchangeSymbol = "BSE:"

port_number = Config.ws_map.get(instrument_name, None)
assert port_number is not None, f"No Port Number Configured for '{instrument_name}'"
...
if ltp is None:
    raise Exception(f"Unable to Retrieve LTP {strikeList[0]}")

# SENSEX strikes are at 100-point intervals
for i in range(-6, 6):
