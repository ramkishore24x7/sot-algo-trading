...
# bot_name = __file__.split("/")[-1].split(".")[0].upper()
...
parser = argparse.ArgumentParser(description='SOT BOT for Algo Trading on Fyers')
...
enterFewPointsAbove = True#args.efpa
...
if stock_option_input.upper().startswith("NIF"):
    ...
    exception_content = f"{Clock.tictoc()} Nifty || MidCPNifty || BankNifty || FinNifty || BajFinance ONLY. Received '{stock_option_input}'"
...
# below is a smaple qty only for calculation of PnL
...
log_file = Config.logger_path+"/"+name + stock_option.replace("NSE:","") + "_" + str(date.today()) + "_" + str(uuid.uuid4()) + ".log"
...
# accounts = Config.accounts_breakout_strategy_paper_trade if onCrossingAbove else Config.accounts_range_strategy_paper_trade
...
def takeEntry(option, current_entry_price):
    global stop_loss
    global lazy_stoploss
    global stop_loss_aggressive_trailing
    global qty
    global remaining_qty
    global entered_trade
    global avg_price
    global entry_price
    global total_trading_qty
    global PnL
    global re_entered
    global peak_profit
    global peak_gain
    ...
    isAveraged = False
    avg_price = entry_price if onCrossingAbove else current_entry_price
    position.entry_price = current_entry_price
    
    send_order_placement_erros("ENTER POSITION",trade_manager.enter_position(position,current_entry_price))

    total_trading_qty,remaining_qty = qty,qty
    entered_trade = True
    sold_aggressive_postions = False
    sold_lazy_postions = False
    
    traded = False
    ltpHigh = current_entry_price
    soldAtTarget1 = False
    soldAtTarget2 = False
    customSpinner = itertools.cycle(["-", "/", "|", "\\"])
    upper_range = (entry_price-static_stoploss) * 0.72
    lower_range = (entry_price-static_stoploss) * 0.35

    averaged_almost_target1 = (target1 - entry_price) * 0.75
    averaged_almost_target2 = (target2 - target1) * 0.75
    averaged_almost_target3 = (target3 - target2) * 0.75

    almost_target1 = (target1 - entry_price) * 0.75
    almost_target2 = (target2 - target1) * 0.75
    almost_target3 = (target3 - target2) * 0.75

    logger.warning(f"almost_target1: {almost_target1} i.e {entry_price + almost_target1}")
    logger.warning(f"almost_target2: {almost_target2} i.e {target1 + almost_target2}")
    logger.warning(f"almost_target3: {almost_target3} i.e {target2 + almost_target3}")

    logger.warning(f"averaged_almost_target1: {averaged_almost_target1} i.e breakeven-sl at target1 is {entry_price + averaged_almost_target1}")
    logger.warning(f"averaged_almost_target2: {averaged_almost_target2} i.e breakeven-sl at target2 is {target1 + averaged_almost_target2}")
    logger.warning(f"averaged_almost_target3: {averaged_almost_target3} i.e breakeven-sl at target3 is {target2 + averaged_almost_target3}")
    
    message_sent = False
    while not traded:
        cmp = ensureGetLTP(option)
        live_pnl = (cmp - avg_price) * remaining_qty
        # gain = round(cmp - avg_price)
        gain = round(cmp - current_entry_price)

        print("Live: ",option, " : PnL: ",live_pnl," INR ",next(spinner), end="\r", flush=True)
        ...
        try:
            if cmp > ltpHigh:
                ltpHigh = cmp
                peak_profit = live_pnl if live_pnl > peak_profit else peak_profit
                peak_gain = gain if gain > peak_gain else peak_gain
                logger.debug(f"{option} : Higher High: {ltpHigh}/- | Current PnL: {live_pnl}/- | Peak Profit: {peak_profit}/-")
            else:
                logger.debug(f"{option} : CMP: {cmp}/-... | Higher High: {ltpHigh}/- | Current PnL: {live_pnl}/- | Peak Profit: {peak_profit}/-")

            if round(cmp) <= stop_loss or not Clock.is_time_less_than(trade_exit_hour,trade_exit_minute):

                send_order_placement_erros("SQUARE-OFF POSITION",trade_manager.square_off_position(position,cmp))

                logger.critical(f"stop_loss hit! closed at {stop_loss}/-; Highest LTP: {ltpHigh}/-")
                print("Closed: ",option)
                smiley = "🤑" if live_pnl > 0 else "🤐 khata khata hatha vidhi!"
                asyncio.run(send_message(f"{gain} Points! {smiley} | Peak Gain: {peak_gain}"))
                # traded = True
                if not re_entered and not soldAtTarget1 and re_entry:
                    check_re_entry(option)
                elif re_entered and not soldAtTarget1:
                    logger.critical("Phew! SL hit for the second time, no more re-entries.")
            elif round(cmp) <= stop_loss_aggressive_trailing and not sold_aggressive_postions:
                ...
                sold_aggressive_postions = True
                logger.critical(f"stop_loss_aggressive_trailing is hit! Closed at {stop_loss_aggressive_trailing}/-")
            elif round(cmp) <= lazy_stoploss and not sold_lazy_postions:
                
                send_order_placement_erros("SQUARE-OFF LAZY POSITION",trade_manager.square_off_position_lazy_trail(position,cmp))

                sold_lazy_postions = True
                logger.critical(f"lazy_stoploss is hit! Closed at {lazy_stoploss}/-")
            elif second_entry_price is not None and static_stoploss < int(cmp) < second_entry_price and not isAveraged and not isBreakoutStrategy and not onCrossingAbove: #ram updated code
                
                send_order_placement_erros("AVERAGE POSITION",trade_manager.average_position(position,cmp))
    
                isAveraged = True
                total_trading_qty,remaining_qty = remaining_qty + qty2, remaining_qty + qty2
                avg_price = ((qty*avg_price)+(qty2*cmp))/(qty+qty2) #ram updated code not to shift the average price upon averaging. This is to ensure for accounts not averaged end in loss when returning from target1. demat will take care of individual account avg price for calcs
                logger.critical(f"Averaged at {cmp}/-")
            
            # almost targets and averaged i.e a range based call
            elif play_safe and round(cmp) >= avg_price + averaged_almost_target1 and isAveraged and stop_loss < avg_price: #this is a play-safe if price hits lower entry levels and potentially moved 75% of move towards target 1
                stop_loss = avg_price if precise_trailing else avg_price + 3
                logger.critical(f"Trailed stop_loss to Breakeven as CMP >= avg_price + {averaged_almost_target1} and position is averaged.  CMP: {cmp}/- ; Average Price: {avg_price}/- ; Next Target: {target1}/- ; stop_loss: {stop_loss}/-")
            elif play_safe and second_entry_price is not None and round(cmp) >= avg_price + averaged_almost_target1 and not isAveraged and stop_loss < second_entry_price: #this is a play-safe if price hits lower entry levels and potentially moved 75% of move towards target 1
                stop_loss =  second_entry_price if precise_trailing else second_entry_price + 3
                logger.critical(f"Trailed stop_loss to Breakeven as CMP >= avg_price + {averaged_almost_target1} and position is averaged.  CMP: {cmp}/- ; Average Price: {avg_price}/- ; Next Target: {target1}/- ; stop_loss: {stop_loss}/-")
            elif play_safe and round(cmp) >= target1 + averaged_almost_target2 and isAveraged and stop_loss < target1: #this is a play-safe if price hits lower entry levels and potentially moved 75% of move towards target 2
                stop_loss = target1 if precise_trailing else target1 + 3
                logger.critical(f"Trailed stop_loss to Target1 as CMP >= avg_price + {averaged_almost_target2} and position is averaged.  CMP: {cmp}/- ; Average Price: {avg_price}/- ; Next Target: {target3}/- ; stop_loss: {stop_loss}/-")
            elif play_safe and round(cmp) >= target2 + averaged_almost_target3 and isAveraged and stop_loss < target2: #this is a play-safe if price hits lower entry levels and potentially moved 75% of move towards target 3
                stop_loss = target2 if precise_trailing else target2 + 3
                logger.critical(f"Trailed stop_loss to Target2 as CMP >= avg_price + {averaged_almost_target3} and position is averaged.  CMP: {cmp}/- ; Average Price: {avg_price}/- ; Next Target: {target3}/- ; stop_loss: {stop_loss}/-")
            
            # almost targets and not averaged, range when not averaged or breakout **but a range based call**
            elif play_safe and round(cmp) >= avg_price + almost_target1 and not isAveraged and stop_loss < avg_price: #this is a play-safe if price hits lower entry levels and potentially moved 90% of move towards target 1
                stop_loss =  avg_price if precise_trailing else avg_price + 3
                logger.critical(f"Trailed stop_loss to Breakeven as CMP >= avg_price + {almost_target1} and position is not averaged.  CMP: {cmp}/- ; Average Price: {avg_price}/- ; Next Target: {target1}/- ; stop_loss: {stop_loss}/-")
            elif play_safe and round(cmp) >= target1 + almost_target2 and not isAveraged and stop_loss < target1: #this is a play-safe if price hits lower entry levels and potentially moved 90% of move towards target 2
                stop_loss = target1 if precise_trailing else target1 + 3
                logger.critical(f"Trailed stop_loss to Target1 as CMP >= avg_price + {almost_target2} and position is not averaged.  CMP: {cmp}/- ; Average Price: {avg_price}/- ; Next Target: {target3}/- ; stop_loss: {stop_loss}/-")
            elif play_safe and round(cmp) >= target2 + almost_target3 and not isAveraged and stop_loss < target2: #this is a play-safe if price hits lower entry levels and potentially moved 90% of move towards target 3
                stop_loss = target2 if precise_trailing else target2 + 3
                logger.critical(f"Trailed stop_loss to Target2 as CMP >= avg_price + {almost_target3} and position is not averaged.  CMP: {cmp}/- ; Average Price: {avg_price}/- ; Next Target: {target3}/- ; stop_loss: {stop_loss}/-")           

            elif round(cmp) >= target1 - aggressive_trailing_points1 and stop_loss_aggressive_trailing < avg_price:
                # stop_loss_aggressive_trailing = avg_price + 3 if not isBreakoutStrategy else entry_price + 3
                if precise_trailing:
                    stop_loss_aggressive_trailing = avg_price if not onCrossingAbove else entry_price
                else:
                    stop_loss_aggressive_trailing = avg_price + 3 if not onCrossingAbove else entry_price + 3 #ram updated code
                logger.critical(f"Trailed stop_loss_aggressive_trailing to Breakeven. as CMP>=target1 - {aggressive_trailing_points1}.  CMP: {cmp}/- ; Average Price: {avg_price}/- ; Next Target: {target1}/- ; stop_loss_aggressive_trailing: {stop_loss_aggressive_trailing}/-")
            
            elif round(cmp) >= target1 and cmp < target2 and not soldAtTarget1:
                send_order_placement_erros("BOOK TARGET1",trade_manager.book_target1(position,cmp))
                # status = []
                # for demat in demats:
                #     status.append(demat.position_open)
                # if not any(status):
                #     print("Closed: ",option)
                #     traded = True
                #     logger.critical(f"All Positions have been closed at target1, stopping SOT_BOT!")
                soldAtTarget1 = True
                asyncio.run(send_message(f"😍 {gain} Points..."))
                stop_loss = avg_price if precise_trailing else avg_price + 3
                if onCrossingAbove: # added to accomodate lazy_stoploss!
                    stop_loss = stop_loss - 2 
                lazy_stoploss = stop_loss
                logger.critical(f"Trailed lazy_stoploss to Breakeven/cost as cmp >= target1. CMP: {cmp}/- ; Average Price: {avg_price}/- ; Next Target: {target2}/- ; Stop-loss: {lazy_stoploss}/-")
                logger.critical(f"Trailed Stop-loss to Breakeven as cmp >= target1. CMP: {cmp}/- ; Average Price: {avg_price}/- ; Next Target: {target2}/- ; Stop-loss: {stop_loss}/-")
            elif round(cmp) >= target2 - aggressive_trailing_points2 and soldAtTarget1 and not soldAtTarget2 and stop_loss_aggressive_trailing != target1:
                stop_loss_aggressive_trailing = target1
                logger.critical(f"Trailed stop_loss_aggressive_trailing to Target1 as CMP>=target2 - {aggressive_trailing_points2}.  CMP: {cmp}/- ; Average Price: {avg_price}/- ; Next Target: {target2}/- ; stop_loss_aggressive_trailing: {stop_loss_aggressive_trailing}/-")
            
            elif round(cmp) >= target2 and cmp < target3 and not soldAtTarget2:
                send_order_placement_erros("BOOK TARGET2",trade_manager.book_target2(position,cmp))
                soldAtTarget2 = True
                stop_loss = target1 if precise_trailing else target1 + 3
                asyncio.run(send_message(f"😍😍 {gain} Points..."))
                logger.critical(f"Trailed Stop-loss to Target1. CMP: {cmp}/- ; Average Price: {avg_price}/- ; Next Target: {target3}/- ; Stop-loss: {stop_loss}/-")
            
            # elif round(cmp) >= target3 - aggressive_trailing_points3 and soldAtTarget1 and soldAtTarget2 and stop_loss_aggressive_trailing != target2:
            elif round(cmp) >= target3 - aggressive_trailing_points3 and stop_loss_aggressive_trailing != target2:
                stop_loss_aggressive_trailing = target2 if precise_trailing else target2 + 3
                logger.critical(f"Trailed stop_loss_aggressive_trailing to Target2 as CMP>=target2 - {aggressive_trailing_points3}.  CMP: {cmp}/- ; Average Price: {avg_price}/- ; Next Target: {target3}/- ; stop_loss_aggressive_trailing: {stop_loss_aggressive_trailing}/-")
            
            elif round(cmp) >= target3:
                send_order_placement_erros("BOOK TARGET3",trade_manager.book_target3(position,cmp))

                logger.critical(f"Target3 hit! closed at {cmp}/-; Highest LTP: {ltpHigh}/-")
                traded = True
                entered_trade = False
                asyncio.run(send_message(f"😍😍😍 {gain} Points!"))
                send_calculated_pnL()
            ...
            logger.error(f"Error @takeEntry: {err}")
            logger.error(f"Error Trace: {traceback.print_exc()}")
            chime.warning()
            time.sleep(1)
...
def getPrevCandle(option,timeframe):
        ...
        try:
            ...
            logger.info(f"getPrevCandle response: {response}")
            lastCandle = response["candles"][-1]
            lastTwoCandles = response["candles"][-2:]
            logger.info(f"lastTwoCandles: {lastTwoCandles}")
            for candle in reversed(lastTwoCandles):
                if isLastMinute(int(round(time.time())), candle[0]):
                    lastCandle = candle
                    break
...
def check_entry(option):
    try:
        ...
        if spot and "CE" in PE_CE:
            ...
            # used only for range calls and first entry
            while not entered_trade and not isBreakoutStrategy and not onCrossingAbove and Clock.is_time_less_than(bot_exit_hour,bot_exit_minute):
                ...
                if second_entry_price <= int(cmp) < entry_price+1:
                    ...
                    333 - 330 < 5: True
                    """
                    # test code for a trade missed by few points
            ...
            # used only for re-entry based on recent loss making position only
            ...
            while not entered_trade and not isBreakoutStrategy and onCrossingAbove and Clock.is_time_less_than(bot_exit_hour,bot_exit_minute):
                ...
                # time.sleep(3) ram updated this code to remove delay as it needs to pool instead of waiting for 3 seconds here
...
if __name__ == '__main__':
    # cmp = 300
    # enter_position(position,cmp)
    # send_order_placement_erros("ENTER POSITION",trade_manager.enter_position(position,cmp))
    # send_order_placement_erros("AVERAGE POSITION",trade_manager.average_position(position,cmp))
    # send_order_placement_erros("BOOK TARGET1",trade_manager.book_target1(position,cmp))
    # send_order_placement_erros("BOOK TARGET2",trade_manager.book_target2(position,cmp))
    # send_order_placement_erros("BOOK TARGET3",trade_manager.book_target3(position,cmp))
